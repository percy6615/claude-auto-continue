...
1. 在 claude.ai 打開要續的對話，從網址列複製 `https://claude.ai/chat/...`。
2. 點擴充圖示開 popup：
   - **預設對話網址**：貼上常用的對話（時段沒填網址時就用這個；可多行）。
   - **預設文字**：新時段的初始文字（例如「繼續」）。
   - **排程時段**：按「＋ 新增時段」可加任意多個。每個時段各自設定：
     - 啟用勾選、觸發時間、要送的文字、（選填）此時段專用的對話網址。
     - 不同時段可以送**不同文字**、指向**不同對話**。
   - 想用偵測到的重置時間，按「重新掃描」後按「＋ 加入為時段」。
   - 按「**Arm 全部**」：所有「已啟用且時間在未來」的時段都會各自排程；過期或沒填時間的會自動略過。
3. 每個時段都有「**立即測試此段**」：用該段的文字＋網址馬上送一次，確認 selector 正確，不用等。
4. 「取消全部」清掉所有排程。

> 每個時段是獨立的 alarm，互不影響。觸發時間預設會比你設定的時間晚約 45 秒（緩衝，可在程式裡的 `bufferSec` 調整）。
